...
describe('API keys table columns', () => {
  test('places group before status and quota in the visible table order', () => {
    ...
    const columnIds = result.current.map((column) => {
      ...
      if ('accessorKey' in column) return column.accessorKey
      return undefined
    })

    expect(columnIds.slice(0, 6)).toEqual([
      'select',
      'name',
      'key',
      'group',
