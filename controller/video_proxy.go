...
func VideoProxy(c *gin.Context) {
	...
	if descriptor == nil {
		...
		descriptor, descriptorErr = legacyVideoContentRequest(task, c.Request.Method)
		if descriptorErr != nil {
			logger.LogWarn(c.Request.Context(), fmt.Sprintf("Failed to resolve legacy video content for task %s: %v", taskID, descriptorErr))
		}
		if descriptor == nil {
...
func legacyVideoContentRequest(task *model.Task, method string) (*relaychannel.TaskContentRequest, error) {
	...
	case constant.ChannelTypeGemini:
		...
		if apiKey == "" {
			return nil, fmt.Errorf("API key not stored for task")
		}
		contentURL, err = getGeminiVideoURL(channel, task, apiKey)
		if err != nil {
			return nil, err
		}
		headers["x-goog-api-key"] = apiKey
	case constant.ChannelTypeVertexAi:
		contentURL, err = getVertexVideoURL(channel, task)
		if err != nil {
			return nil, err
		}
	...
	case constant.ChannelTypeXai:
		...
		contentURL, trustedProviderURL, err = resolveVideoResultURL(baseURL, task.GetResultURL())
		if err != nil {
			return nil, err
		}
...
func getGeminiVideoURL(channel *model.Channel, task *model.Task, apiKey string) (string, error) {
	...
	resp, err := adaptor.FetchTask(baseURL, apiKey, task, channel.GetSetting().Proxy)
	if err != nil {
