...
func TestSendEmailWithAttachmentIncludesInvoiceFile(t *testing.T) {
	server := newFakeSMTPServer(t)
	defer server.close()
	withSMTPSettings(t)
	...
	SMTPToken = "secret"
	SystemName = "New API"
	attachment := []byte("invoice-pdf-content")

	require.NoError(t, SendEmailWithAttachment("Invoice", "receiver@example.com", "<p>issued</p>", "invoice.pdf", attachment))

	select {
	case message := <-server.messages:
		require.Contains(t, message, "Content-Type: multipart/mixed; boundary=")
		require.Contains(t, message, `Content-Disposition: attachment; filename="invoice.pdf"`)
		require.Contains(t, message, base64.StdEncoding.EncodeToString(attachment))
	case <-time.After(2 * time.Second):
		t.Fatal("timed out waiting for SMTP DATA")
	}
}
